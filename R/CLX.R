...
#'
#' @details Implementing testing procedure proposed by Cai, Liu, and Xia (2014) to test
#'    the equality of two sample high dimensional mean vectors under the assumption of
#'    sparsity of signals.
#'
...
CLX <- function(X, Y, alpha, DNAME) {
  ...
  for (i in 1:p) {
    for (j in i:p) {
      ...
      yi <- Ym[i, ]
      yj <- Ym[j, ]
      btheta[i, j] <- (sum((xi * xj - SS[i, j]) ^ 2) +
                         sum((yi * yj - SS[i, j]) ^ 2)) / (n1 + n2)
    }
  }
  ...
  Xn <- as.numeric(Momega >= 2 * log(p) - log(log(p)) - log(pi) -
                     2* log(log(1 / (1 - alpha))))
